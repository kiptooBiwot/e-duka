...
// Use debounce to send a req to the API after 100 milliseconds of no typing
    ...
    <div id="TopMenu" class="w-full bg-[#fafafa] border-b md:block hidden">
      <ul class="flex items-center justify-end text-xs text-[#333333] font-light px-2 h-10 bg-[#fafafa] max-w-[1200px]">
        ...
        <li @mouseenter="isAccountMenu = true" @mouseleave="isAccountMenu = false"
          class="relative flex items-center px-2.5 h-full hover:text-[#ff4646] cursor-pointer"
          ...
          <div id="AccountMeny" v-if="isAccountMenu"
            class="absolute bg-white w-[220px] text-[#333333] z-40 top-[38px] -left-[100px] border-x border-b">
            <div v-if="!user">
              <div class="font-semibold text-[15px] my-4 px-3">Welcome to E-Duka!</div>
            ...
            <ul class="bg-white">
              ...
              <li v-if="user" class="text-[13px] py-2 px-4 w-full hover:bg-gray-200"
                @click.prevent="client.auth.signOut()">
                Sign Out
              </li>
            </ul>
          </div>
    ...
    <div id="MainHeader" class="flex items-center w-full bg-white">
      <div class="flex lg:justify-start justify-between gap-10 max-w-[1150px] w-full px-3 py-5 mx-auto">
        <NuxtLink to="/" class="min-w-[170px]">
          <h2 class="font-extrabold font-title text-4xl"><span class="text-[#F69716]">E-</span><span
              class="text-[#ff4646]">Duka</span></h2>
        </NuxtLink>
          ...
          <div class="relative">
            <div class="flex items-center border-2 border-[#ff4646] rounded-md w-full">
              <input type="text" class="w-full placeholder-gray-400 text-sm pl-3 focus:outline-none"
                placeholder="Kitchen Accessories" v-model="searchItem">
            ...
            <div v-if="items && items.data" class="absolute bg-white max-w-[700px] h-auto w-full">
              <div v-for="item in items.data" :key="item" class="p-1">
                <NuxtLink :to="`/item/${item.id}`" @click.prevent="items.data === false"
                  class="flex items-center justify-between w-full cursor-pointer hover:bg-gray-100">
                  <div class="flex items-center">
                    <img :src="item.url" class="rounded-md" width="40" alt="">
                    <div class="truncate ml-2">{{ item.title }}</div>
                  </div>
                  <div class="truncate">Ksh.{{ item.price / 100 }}</div>
                </NuxtLink>
              </div>
            </div>
          </div>
