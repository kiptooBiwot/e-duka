...
// console.log('PRODUCT as PROP: ', product)
...
<template>
  <div :id="`ProductComponent${product.id}`" class="
            bg-white 
            inline-block 
            rounded 
    ...
    <NuxtLink :to="`/item/${product.id}`">
      ...
      <div id="ProductDetails">
        ...
        <span class="flex items-center justify-start gap-3 px-1 pt-1">
          <span class="text-[#FF6674] font-semibold">Ksh.{{ priceComputed }}</span>
          <span class="text-gray-500 text-sm text-light line-through">Ksh.{{ oldPriceComputed }}</span>
        </span>
